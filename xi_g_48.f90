program correlation
implicit none
complex*16 jj,ep1,ep2,em1,em2,KK,WW,rho1,rho2
complex*16, allocatable :: KpW(:,:), IK(:,:)
real*8 pi,delta1,delta2,beta,fd1,fd2,dg,xi
real*8 w1,w2,q1,q2,lambda,dlambda,g,mu,Z,alpha 
real*8 x,x1,x2,k,dk,ec,fd,w,norm,w_l
...
if(x.eq.x1) rho1 = det(KpW)-det(IK)
if(x.eq.x2) rho2 = det(KpW)-det(IK)
...
if(x.eq.x1) rho1 = rho1 + exp(-beta*ec)*rho1*w_l
if(x.eq.x2) rho2 = rho2 + exp(-beta*ec)*rho2*w_l
